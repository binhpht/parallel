...
cl_mem satelite_buff;
cl_mem pixel_buff;
cl_kernel kernel;
// ## You may add your own variables here ##

const char *source_str =
    "typedef struct {  float red;  float green;  float blue;} color;\n"
    "typedef struct {  float x;  float y;} floatvector;\n"
    "typedef struct {   color identifier;  floatvector position;  floatvector velocity;} satelite;\n"
    "#define SATELITE_COUNT 64 \n"
    "#define SATELITE_RADIUS 3.16f \n"
    "#define WINDOW_WIDTH 1024 \n"
    "__kernel void setidx(__global color *pixels, __global satelite *satelites)\n"
    "{                                                                          \n"
    ...
    "     }                                                                     \n"
    "   } \n"
    " int dstIdx = pixel.y * WINDOW_WIDTH + pixel.x; \n"
    "   pixels[dstIdx] = renderColor;                \n"
    "}                                                                          \n";

// ## You may add your own initialization routines here ##
void print_opencl_compilation_errors(cl_program program)
{
  cl_int cl_retval;
  size_t cl_build_log_length = 0;
  int cl_build_retval = clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG, 0, 0, &cl_build_log_length);
  assert(cl_build_retval == CL_SUCCESS);

  char *cl_build_log = (char *) malloc(cl_build_log_length * sizeof(char));

  cl_build_retval = clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG, cl_build_log_length, cl_build_log, 0);
  assert(cl_build_retval == CL_SUCCESS);

  fprintf(stderr, "CL BUILD ERROR\n%s\n", cl_build_log);

  free((void *) cl_build_log);
}
...
void init() {
  ...
  if (ret != CL_SUCCESS) {
    print_opencl_compilation_errors(program);
    assert(0 == 1);
  }
  // Allocate the Buffers on the Device
  satelite_buff = clCreateBuffer(context, CL_MEM_READ_ONLY, SATELITE_COUNT * sizeof(satelite), NULL, &ret);
  if (ret != CL_SUCCESS)
    printf("Failed creating satelite_buff");
  pixel_buff  = clCreateBuffer(context, CL_MEM_READ_WRITE, SIZE * sizeof(color), NULL, &ret);
  if (ret != CL_SUCCESS)
    printf("Failed creating pixel_buff");
...
void parallelGraphicsEngine() {
  ...
  if (ret != CL_SUCCESS) {
    printf("Failed clEnqueueWriteBuffer satelite_buff");
  }
  ...
  ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), &satelite_buff);
  if (ret != CL_SUCCESS)
    printf("Failed Set clSetKernelArg");

  // Set the global and local w  rk size
  size_t local_work_size[] = {WGS0, WGS1};
