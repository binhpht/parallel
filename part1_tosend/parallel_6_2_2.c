...
void parallelGraphicsEngine(){
    ...
    for(int i = 0 ;i < SIZE; ++i) {
      ...
      for(int j = 0; j < SATELITE_COUNT; ++j) {
            ...
            distance[j] = sqrt(difference.x * difference.x +
                        difference.y * difference.y);
            weight[j] = 1.0f / (distance[j] * distance[j] * distance[j] * distance[j]);
      }

      for(int j = 0; j < SATELITE_COUNT; ++j) {
         if(distance[j] < SATELITE_RADIUS) {
            ...
            weights += weight[j];
            if(distance[j] < shortestDistance){
