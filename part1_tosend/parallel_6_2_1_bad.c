...
void parallelPhysicsEngine(){
   ...
   for (int i = 0; i < SATELITE_COUNT; ++i) {
       ...
       tmpPosition[i].x = satelites[i].position.x - HORIZONTAL_CENTER;
       tmpPosition[i].y = satelites[i].position.y - VERTICAL_CENTER;
   ...
   for(int physicsUpdateIndex = 0; 
      ...
      for(int i = 0; i < SATELITE_COUNT; ++i){
         double distToBlackHoleSquared =
            tmpPosition[i].x * tmpPosition[i].x +
            tmpPosition[i].y * tmpPosition[i].y;
         double distToBlackHole = sqrt(distToBlackHoleSquared);

         // Gravity force
         doublevector normalizedDirection = {
            .x = tmpPosition[i].x / distToBlackHole,
            .y = tmpPosition[i].y / distToBlackHole};
   ...
   for (int i = 0; i < SATELITE_COUNT; ++i) {
       ...
       satelites[i].position.x = tmpPosition[i].x + HORIZONTAL_CENTER;
       satelites[i].position.y = tmpPosition[i].y + VERTICAL_CENTER;
